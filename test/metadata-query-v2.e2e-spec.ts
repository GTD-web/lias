...
describe('MetadataQueryController (e2e)', () => {
    ...
    describe('GET /metadata/employees - 직원 검색', () => {
        ...
        it('정상: 이름으로 검색', async () => {
            const response = await request(app.getHttpServer())
                .get('/metadata/employees?search=김규현')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            ...
            if (response.body.length > 0) {
                const employee = response.body[0];
                expect(employee.name).toContain('김규현');
            }
        });
        ...
        it('정상: 검색어 + 부서 조합 필터', async () => {
            ...
            const response = await request(app.getHttpServer())
                .get(`/metadata/employees?search=김&departmentId=${departmentId}`)
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
    ...
    describe('메타데이터 조회 - 특수 케이스', () => {
        ...
        it('정상: 공백이 포함된 검색어', async () => {
            const response = await request(app.getHttpServer())
                .get('/metadata/employees?search=김 규현')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
