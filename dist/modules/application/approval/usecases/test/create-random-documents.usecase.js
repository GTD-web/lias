...
exports.CreateRandomDocumentsUseCase = void 0;
...
let CreateRandomDocumentsUseCase = class CreateRandomDocumentsUseCase {
    constructor(employeeService, documentService, approvalStepService, documentFormService, documentTypeService, departmentService) {
    ...
    async ensureBasicData() {
        let employees = await this.employeeService.findAll({
            where: {
                department: (0, typeorm_1.In)(['지상-Web']),
            },
        });
    ...
    async createEmployees(departments) {
        const employeeNames = [
            '김철수',
            '이영희',
            '박민수',
    ...
    async createDocumentForms(documentTypes) {
    ...
    async createRandomDocument(employees, documentTypes, index) {
    ...
    async createRandomApprovalSteps(document, employees) {
        const steps = [];
        const agreementCount = Math.floor(Math.random() * 3) + 1;
        const approvalCount = Math.floor(Math.random() * 3) + 1;
        const pickRandomEmployees = (count, excludeIds = new Set()) => {
        ...
        const approvalEmployees = pickRandomEmployees(approvalCount, new Set(agreementEmployees.map((e) => e.employeeId)));
        const implementationEmployees = pickRandomEmployees(1, new Set([...agreementEmployees, ...approvalEmployees].map((e) => e.employeeId)));
        ...
        for (const emp of agreementEmployees) {
            steps.push(await this.approvalStepService.save({
        ...
        else if (document.status === approval_enum_1.ApprovalStatus.REJECTED) {
    ...
    getRandomStatus() {
        const statuses = [approval_enum_1.ApprovalStatus.PENDING, approval_enum_1.ApprovalStatus.APPROVED, approval_enum_1.ApprovalStatus.REJECTED];
        const weights = [0.5, 0.45, 0.05];
        ...
        return approval_enum_1.ApprovalStatus.PENDING;
    }
    getRandomDate() {
        const now = new Date();
...
exports.CreateRandomDocumentsUseCase = CreateRandomDocumentsUseCase;
exports.CreateRandomDocumentsUseCase = CreateRandomDocumentsUseCase = __decorate([
    ...
    __metadata("design:paramtypes", [employee_service_1.DomainEmployeeService,
        ...
        document_form_service_1.DomainDocumentFormService,
        document_type_service_1.DomainDocumentTypeService,
        department_service_1.DomainDepartmentService])
], CreateRandomDocumentsUseCase);
//# sourceMappingURL=create-random-documents.usecase.js.map