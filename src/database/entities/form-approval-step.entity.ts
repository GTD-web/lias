...
export class FormApprovalStep {
    ...
    @Column({ comment: '결재자 지정 방식 (ex. Enum(USER, DEPARTMENT_POSITION, POSITION, TITLE))' })
    approverType: string;

    @Column({ comment: '결재자 지정 값 (ex.  userId, positionCode, titleCode)' })
    approverValue: string;

    @Column({ comment: 'DEPARTMENT_POSITION인 경우 부서 범위 타입  (ex. Enum(SELECTED, DRAFT_OWNER, NONE))' })
    departmentScopeType: string;

    @Column({ type: 'jsonb', comment: '결재 단계 조건 표현식' })
